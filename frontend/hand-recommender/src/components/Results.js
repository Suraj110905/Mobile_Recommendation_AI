...
function Results({ results, handData, onRestart }) {
  ...
  function PhoneCard({ phone, rank }) {
    ...
    const flipkartUrl = "https://www.flipkart.com/search?q=" + encodeURIComponent(phone.phone_model);
    const amazonUrl   = "https://www.amazon.in/s?k=" + encodeURIComponent(phone.phone_model);
    ...
    return (
      <div style={{
        ...
        <div style={S.buyRow}>
          <a href={flipkartUrl} target="_blank" rel="noreferrer"
    ...
    if (compareList.length < 2) return (
      <div style={S.comparePlaceholder}>
        <p style={{ color: "#6b7280", fontSize: "14px", margin: 0 }}>
          Select at least 2 phones above to compare.
        </p>
      </div>
    );
    ...
    return (
      <div style={S.tableWrap}>
        <div style={{ ...S.tableRow, background: "#f8fafc", borderRadius: "10px 10px 0 0" }}>
          ...
          {compareList.map(phone => (
            <div key={phone.phone_model} style={S.phoneCol}>
              ...
              <div style={{ fontSize: "13px", fontWeight: "600", color: "#1f2937" }}>{phone.model}</div>
              <button style={S.removeBtn} onClick={() => toggleCompare(phone)}>✕ Remove</button>
            </div>
          ))}
        </div>
        ...
        {SPECS.map((spec, si) => {
          ...
          return (
            <div key={spec.key} style={{ ...S.tableRow, background: si % 2 === 0 ? "#fff" : "#fafafa" }}>
              <div style={S.specCol}>
        ...
        {(() => {
          ...
          const winner = compareList[scores.indexOf(Math.max(...scores))];
          return (
            <div style={{ ...S.tableRow, background: "#eef2ff", borderRadius: "0 0 10px 10px", borderTop: "2px solid #6366f1" }}>
              <div style={{ ...S.specCol, fontWeight: "700", color: "#6366f1" }}>Overall winner</div>
              {compareList.map(phone => (
                <div key={phone.phone_model} style={{
                  ...S.phoneCol, fontWeight: "700", fontSize: "14px",
  ...
  return (
    <div>
      ...
      <p style={S.subtext}>
        Based on your <strong>{handData && handData.hand_size}</strong> hand size.
      ...
      <div style={S.bestBanner}>
        <div>
          ...
          <p style={{ fontSize: "22px", fontWeight: "700", margin: "0 0 4px" }}>{results.best_phone}</p>
      ...
      <div style={S.compareSection}>
        <div style={S.compareHeader}>
          <div>
            ...
            <span style={S.compareCount}>{compareList.length}/{MAX_COMPARE} selected</span>
          </div>
          <button
            style={{
              ...S.viewCompareBtn,
          ...
          <div style={S.selectedPills}>
            {compareList.map(phone => (
              <div key={phone.phone_model} style={S.pill}>
                ...
                <button style={S.pillRemove} onClick={() => toggleCompare(phone)}>✕</button>
              </div>
            ))}
          </div>
        )}
      ...
      <button style={S.restartBtn} onClick={onRestart}>Start Over</button>
...
const S = {
  ...
  compareSection:      { border: "1px solid #e5e7eb", borderRadius: "14px", padding: "16px", marginBottom: "24px", background: "#fafafa" },
  compareHeader:       { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" },
  compareSectionTitle: { fontSize: "15px", fontWeight: "600", color: "#1f2937", marginRight: "8px" },
  compareCount:        { fontSize: "12px", color: "#6b7280", background: "#e5e7eb", padding: "2px 8px", borderRadius: "20px" },
  viewCompareBtn:      { padding: "8px 16px", background: "linear-gradient(135deg, #6366f1, #8b5cf6)", color: "#fff", border: "none", borderRadius: "8px", fontSize: "13px", fontWeight: "600" },
  selectedPills:       { display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "12px" },
  pill:                { display: "flex", alignItems: "center", gap: "6px", background: "#eef2ff", border: "1px solid #c7d2fe", borderRadius: "20px", padding: "4px 10px" },
  pillText:            { fontSize: "13px", color: "#4338ca", fontWeight: "500" },
  pillRemove:          { background: "none", border: "none", color: "#6366f1", cursor: "pointer", fontSize: "12px", padding: "0", lineHeight: 1 },

  comparePlaceholder: { padding: "20px", textAlign: "center", background: "#fff", borderRadius: "10px", border: "1px dashed #e5e7eb" },
  tableWrap:  { borderRadius: "10px", overflow: "hidden", border: "1px solid #e5e7eb" },
  tableRow:   { display: "grid", gridTemplateColumns: "140px repeat(3, 1fr)", padding: "10px 12px", gap: "8px", alignItems: "center", borderBottom: "0.5px solid #f3f4f6" },
  specCol:    { fontSize: "12px", color: "#6b7280" },
  phoneCol:   { textAlign: "center", padding: "4px 6px" },
  removeBtn:  { display: "block", margin: "4px auto 0", background: "none", border: "none", color: "#ef4444", fontSize: "11px", cursor: "pointer", padding: 0 },
  ...
  rankBadge:     { display: "inline-block", padding: "4px 12px", borderRadius: "20px", fontSize: "12px", fontWeight: "600" },
  compareToggle: { padding: "6px 12px", borderRadius: "8px", border: "1px solid #e5e7eb", fontSize: "12px", fontWeight: "600", transition: "all 0.15s" },
  ...
  buyBtn: { flex: 1, padding: "10px", borderRadius: "8px", fontSize: "13px", fontWeight: "600", textAlign: "center", textDecoration: "none", cursor: "pointer", display: "block" },

  restartBtn: { width: "100%", padding: "14px", border: "2px solid #e5e7eb", borderRadius: "10px", background: "#fff", fontSize: "16px", fontWeight: "600", cursor: "pointer", color: "#374151", marginTop: "8px" },
};
